...
public class CommandExecutorTablePanel extends SharedTablePanel
{
  ...
  public CommandExecutorTablePanel(final ArchiveConsumerServiceImpl archiveService)
  {
  ...
  public void addEntry(final Identifier name, final ArchivePersistenceObject comObject)
  {
    ...
    final CommandDetails commandDetails = (CommandDetails) comObject.getObject();

    final String pid = commandDetails.getPid() == null ? "not started yet"
        : commandDetails.getPid().toString();
    final String exitCode = commandDetails.getExitCode() == null ? "not completed yet"
  ...
  public void updateExitCode(final Long objInstId, final int exitCode)
  {
    try {
      semaphore.acquire();
    } catch (final InterruptedException ex) {
    ...
    try {
      final int index = findIndex(objInstId);
  ...
  public int findIndex(final Long objInstId) throws NoSuchElementException
  {
  ...
  public void defineTableContent()
  {
    ...
    final String[] tableCol = new String[]{
      "Obj Inst Id", "command", "PID", "exitCode"};
