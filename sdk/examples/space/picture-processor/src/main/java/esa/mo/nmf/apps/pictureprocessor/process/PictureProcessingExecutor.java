...
public class PictureProcessingExecutor {
    ...
    public boolean processPicture(Path picture) {
        ...
        LOG.info("Process " + processRequestId + " is starting. It will last at least " + minDurationSeconds +
            "s and at most " + maxDurationMillis + "ms");
    ...
    private static OutputStream initLogStream(Path fileName) {
        Path logFileName = createDirectoriesIfNotExist(Paths.get(LOG_PATH)).resolve(logFileName(stripFileNameExtension(
            fileName)));
