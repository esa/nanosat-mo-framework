...
public class OrbitHandler {
    ...
    public Pass getPassTime(double latitude, double longitude, double maxAngle, TimeModeEnum timeMode,
        ...
        if (timeMode != TimeModeEnum.ANY) {
            ...
            overpassDetector = BooleanDetector.andCombine(timeModeDetector, overpassDetector).withHandler(pass);
        } else {
            overpassDetector = BooleanDetector.orCombine(overpassDetector).withHandler(pass);
        }
