 ...
 * @author Cezar Suteu
 */
@ISimulatorDeviceData(descriptors={ "double:latitude [deg]",
                                    "double:longitude [deg]",
                                    "double:altitude [deg]",
                                    "double:groundStationESOC_Elevation [deg]",
                                    "double:groundStationESOC_Azimuth [deg]",
                                    "String:satsInView"})
    ...
    public static class FirmwareReferenceOEM16 {
        ...
        public static double DDDMMpMMMM2degrees(String DDMMpMMMM) {
            if (DDMMpMMMM.length() == 10) {
                //DDDMMpMMMM sentence
                return Double.parseDouble(DDMMpMMMM.substring(0, 3)) + (Double.parseDouble(DDMMpMMMM.substring(3, 5)) + Double.parseDouble(DDMMpMMMM.substring(6, 10)) / 1000) / 60;
            } else {
                return 0;
            }
        }
        ...
        public static double DDDMMpMMMMMMM2degrees(String DDDMMpMMMMMMM) {
            if (DDDMMpMMMMMMM.length() == 13) {
                return Double.parseDouble(DDDMMpMMMMMMM.substring(0, 3)) + (Double.parseDouble(DDDMMpMMMMMMM.substring(3, 5)) + Double.parseDouble(DDDMMpMMMMMMM.substring(6, 13)) / 1000000) / 60;
            } else {
                return 0;
            }
        }
