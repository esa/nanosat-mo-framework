...
public class OrekitCore {
    ...
    public enum ATTITUDE_MODE {
        SUN_POINTING, NADIR_POINTING, TARGET_TRACKING, LOF_TARGET, LOF_TARGET_SPIN, BDOT_DETUMBLE;
    }
    ...
    LinkedList<GPSSatellite> gpsConstellation;
    ...
    private double getDecimalYear(AbsoluteDate extrapDate) {
        try {
            return extrapDate.getComponents(0).getDate().getYear() + extrapDate.getComponents(0).getDate().getDayOfYear() / 365.0;
    ...
    private String getETAFromAbsDate(AbsoluteDate date) {
        ...
        if (seconds > 0) {
            ...
            return result1.substring(5, result1.length() - 4) + "--ETA=>" + String.valueOf(hours) + "h" + minutes + "m" + seconds + "s";
        } else {
            return "Passed";
        }
    }

    public boolean parseTLEFromStrings(byte[] date, String tle1, String tle2, StringBuilder exception) {
        boolean TLEOk = false;
        logger.log(Level.INFO, "1Trying to parse [\n" + tle1 + "\n" + tle2 + "\n], size is [" + (tle1.length() + tle2.length()) + " bytes]");
        try {
    ...
    public boolean parseTLEFromBytes(byte[] date, StringBuilder tle1, StringBuilder tle2) {
        logger.log(Level.INFO, "Trying to parse [" + date.toString() + "], size is [" + date.length + "]");
    ...
    public void setNewTLEs(SimulatorHeader simulatorHeader) {
        try {
            ...
            TLEPropagator tlePropagator = TLEPropagator.selectExtrapolator(initialTLE, attitudesSequence, 6.0);
    ...
    public void changeAttitudeLof(double x, double y, double z, double rate) {
        ...
        try {
            this.lofTracking = new LofOffset(this.inertialFrame, LOFType.LVLH, RotationOrder.XYZ, x_rad, y_rad, z_rad);
            this.spinStabilized = new SpinStabilized(lofTracking, this.extrapDate, Vector3D.PLUS_K, FastMath.toRadians(rate));
            if (rate > 0) {
    ...
    private void appendIfExists(final StringBuffer path, final String directory) {
        ...
        try {
            ...
            final URL url = classLoader.getResource(directory);
            if (url != null) {
                if (path.length() > 0) {
    ...
    public OrekitCore(double a, double e, double i, double omega, double raan, double lM, SimulatorHeader simulatorHeader, Logger logger, SimulatorNode simulatorNode) throws OrekitException {
        ...
        this.initialDate = new AbsoluteDate(simulatorHeader.getYearStartDate(), simulatorHeader.getMonthStartDate(), simulatorHeader.getDayStartDate(), simulatorHeader.getHourStartDate(), simulatorHeader.getMinuteStartDate(), simulatorHeader.getSecondStartDate(), utc);
        ...
        this.initialOrbit = new KeplerianOrbit(this.a, this.e, this.i, this.omega, this.raan, this.lM, PositionAngle.MEAN, inertialFrame, initialDate, mu);

        //Attitude providers
        this.sun = CelestialBodyFactory.getSun();
        this.sunPointing = new CelestialBodyPointed(celestialFrame, sun, Vector3D.PLUS_I, Vector3D.MINUS_J, Vector3D.PLUS_K);
        this.nadirPointing = new NadirPointing(inertialFrame, earth);
        this.dayObservationLaw = new LofOffset(initialOrbit.getFrame(), LOFType.VVLH,
            RotationOrder.XYZ, FastMath.toRadians(20), FastMath.toRadians(40), 0);
        this.targetGeo = new GeodeticPoint(FastMath.toRadians(DARMSTADT_LATITUDE), FastMath.toRadians(DARMSTADT_LONGITUDE), 0);
        ...
        // Station
        this.stationESOC = new GeodeticPoint(FastMath.toRadians(DARMSTADT_LATITUDE), FastMath.toRadians(DARMSTADT_LONGITUDE), DARMSTADT_ALTITUDE);
        ...
        if (!simulatorHeader.isUpdateInternet())
        {
            ...
            SimulatorNode.handleResourcePath(simulatorNode.getGPSOpsFile().getName(), logger, getClass().getClassLoader());
        }
        else
        {
    ...
    private Rotation getLofRotation() {
        /*
        try {
            return this.spacecraftState.getFrame().getTransformTo(this.lof, this.extrapDate).getRotation();
        } catch (OrekitException ex) {
            Logger.getLogger(OrekitCore.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    private double getGSAzimuth(SpacecraftState s) {
        try {
            final double azimuth = this.staESOCFrame.getAzimuth(s.getPVCoordinates().getPosition(), s.getFrame(), s.getDate());
    ...
    private double getGSElevation(SpacecraftState s) {
        try {
            final double trueElevation = this.staESOCFrame.getElevation(s.getPVCoordinates().getPosition(),
    ...
    private AbsoluteDate findNextSignalEvent(boolean isAOS) {
        ...
        while (++i < maxSteps) {
            ...
            //logger.log(Level.INFO,"Step ["+i+"], time ["+tempExtrapDate.toString()+"],"+positionThen.toString()+" elevation["+getGSElevation(tempSpacecraftState)+"]");
            boolean tempInViewRange = isInGSView(tempSpacecraftState);
            if ((!tempInViewRange && isAOS)
                || (tempInViewRange && !isAOS)) {
            ...
            if (allowCrossingLock) {
                if ((tempInViewRange && isAOS)
                    ...
                    if (debug1 || localDebug) {
                        logger.log(Level.INFO, nodeDetectStr + " found @ [" + tempExtrapDate.toString() + "], step [" + i + "]");
                    }
                ...
                public void run() {
                    ...
                    for (GPSSatellite t:gpsConstellation)
                    {
                        try {
                            ...
                            //
                            double distance=Vector3D.distance(gpsCurrentSCState.getPVCoordinates().getPosition(), t.getState().getPVCoordinates().getPosition());
                            
                            GPSSatInView tempGPSSatInView=new GPSSatInView(t.name,distance);
                            double elevation=0,azimuth=0;
                            try {
                                elevation=opsSatCurrentFrame.getElevation(t.getState().getPVCoordinates().getPosition(),t.getState().getFrame(), t.getState().getDate());
                                azimuth=opsSatCurrentFrame.getAzimuth(t.getState().getPVCoordinates().getPosition(),t.getState().getFrame(), t.getState().getDate());
                            } catch (OrekitException ex) {
                                Logger.getLogger(OrekitCore.class.getName()).log(Level.SEVERE, null, ex);
                            }
    ...
    public GPSSatInViewScience getSatsInViewScience()
    {
        ...
        if (result.size()>0){
            ...
            {
                if (sat.distance>maxDistance) maxDistance=sat.distance;
                if (sat.distance<minDistance) minDistance=sat.distance;
                if (sat.getElevation()>maxElevation) maxElevation=sat.getElevation();
                if (sat.getElevation()<minElevation) minElevation=sat.getElevation();
        ...
        return new GPSSatInViewScience(minDistance, maxDistance, minElevation, maxElevation, avgDistance, avgElevation, stdDevDistance, stdDevElevation);
    ...
    public void processPropagateStep(double timeStep) throws OrekitException {
        //logger.log(Level.INFO,"Timestep is ["+timeStep+"]");
        ...
        if (!propagatingConstellation) processGPSConstellationPropagateStep();
        ...
        Vector3D sunInert = sunPVProvider.getPVCoordinates(spacecraftState.getDate(), spacecraftState.getFrame()).getPosition();
        ...
        //System.out.println(sunX+";"+sunY+";"+sunZ);
        ...
        synchronized (magneticFieldVectorLock) {
            ...
            this.magneticFieldVector = this.geoMagneticField.calculateField(FastMath.toDegrees(newPosition.getLatitude()), FastMath.toDegrees(newPosition.getLongitude()), newPosition.getAltitude() / 1000.0).getFieldVector().toArray();
        }
        ...
        if (!hasAOS) {
            ...
            new Thread() {
                @Override
                public void run() {
                    nextAOS = findNextSignalEvent(true);
                }
            }.start();
        }
        if (!hasLOS) {
            ...
            new Thread() {
                @Override
                public void run() {
                    nextLOS = findNextSignalEvent(false);
                }
            }.start();
        }
*/
    ...
    {
        Rotation test=new Rotation(RotationOrder.YXZ,RotationConvention.FRAME_TRANSFORM,FastMath.toRadians(-yaw),FastMath.toRadians(pitch),FastMath.toRadians(-roll));
    ...
    public GeodeticPoint getGeodeticPoint(SpacecraftState state) {
        try {
            GeodeticPoint point = this.earth.transform(state.getOrbit().getPVCoordinates().getPosition(), inertialFrame, this.initialDate.shiftedBy(timeElapsed));
            return point;
        } catch (OrekitException ex) {
            Logger.getLogger(OrekitCore.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public GeodeticPoint getGeodeticPoint() {
        try {
            //TimeStampedPVCoordinates coord=this.spacecraftState.getPVCoordinates(new TopocentricFrame(this.earth,new GeodeticPoint(0, 0, 0),"earth")).
            //TopocentricFrame earthFrame=new TopocentricFrame(this.earth,new GeodeticPoint(0, 0, 0),"earth");
            GeodeticPoint point = null;
            if (this.spacecraftState != null) {
                point = this.earth.transform(this.spacecraftState.getOrbit().getPVCoordinates().getPosition(), inertialFrame, this.initialDate.shiftedBy(timeElapsed));
            }
            return point;
        } catch (OrekitException ex) {
            Logger.getLogger(OrekitCore.class.getName()).log(Level.SEVERE, null, ex);
