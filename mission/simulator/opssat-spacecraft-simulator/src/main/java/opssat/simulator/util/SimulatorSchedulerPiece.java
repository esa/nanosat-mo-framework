...
public class SimulatorSchedulerPiece implements Serializable {
    ...
    {
        return getDDDDDHHMMSSmmmFromMillis(time)+CommandDescriptor.SEPARATOR_DATAFILES+String.format("%019d",time)+CommandDescriptor.SEPARATOR_DATAFILES+internalID+CommandDescriptor.SEPARATOR_DATAFILES+argumentTemplateDescription;
    }
    
    public String getSchedulerOutput()
    {
        String argTDescriptionTabbed;
        return String.format("%s  %-4s  %-12s executed %-5s   | ",getDDDDDHHMMSSmmmFromMillis(time),internalID,argumentTemplateDescription,executed);
