...
public class MultiThreadedSocketServer extends Thread {
    ...
    private HashMap<ClientServiceThreadSender, LinkedList<String>> hMapManualCommandsList;//this is a hashmap which makes connection between sender and receiver threads for determining whether a command result should be sent to a specific client (in case of multi-client operation)
        ...
        for (ClientServiceThreadSender s : clientSockets) {
            if (s.getMyClientSocket().isConnected()) {
                ...
                if (data instanceof LinkedList) {
                    ...
                    {
                        logger.log(Level.ALL,"Put list of ["+((LinkedList) data).get(0).getClass().getName()+"] for client ["+s.toString()+"]");
                        /*
                        if (!s.isListSent() && ((LinkedList) data).get(0) instanceof CommandDescriptor) {
                            logger.log(Level.ALL, "Set list to true");
                            s.setListSent(true);
                    ...
                    if (data instanceof CommandResult)
                    {
                        ...
                        if (hasBeenRequested) tempList.remove(testString);
                        doSendData=hasBeenRequested;
                    }
                    if (doSendData) s.putDataToClient(data);
                }
            } else {
    ...
    public void run() {
        ...
        if (currentTries >= MAX_PORTS_OPEN) {
            logger.log(Level.SEVERE, "Could not create server socket from port [" + DEFAULT_SOCKET_PORT + "] up to port [" + targetPort + "]. Total tries [" + currentTries + "]. Quitting.");
            System.exit(-1);
        }
    ...
    class ClientServiceThreadReceiver extends Thread {
        ...
        public void run() {
            Thread.currentThread().setName("sim-"+this.getClass().getSimpleName()+"-"+myClientSocket.getInetAddress().getHostName());
            ...
            try {
                ...
                while (m_bRunThread) {
                    ...
                    {
                        logger.log(Level.INFO, "Disconnected Client Address - " + myClientSocket.getInetAddress().getHostName());
                        m_bRunThread=false;
                    }
                    ...
                    if (clientCommand instanceof Integer) {
                        ...
                        if (clientCommand!=null)
                        {
                            ...
                            this.parent.putDataOnForAllClients("OnServer;UserInput;" + CommandDescriptor.makeConsoleDescriptionForObj(clientCommand));
