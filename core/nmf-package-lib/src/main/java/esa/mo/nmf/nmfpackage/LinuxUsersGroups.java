...
public class LinuxUsersGroups {
    ...
    public static void createUser(final String username, final String password,
                                  final boolean withGroup, final String extraGroups) throws IOException {
        ...
        final String cmd1 = "useradd -h";
        final String out1 = shell.runCommandAndGetOutputMessageAndError(cmd1);

        if (out1.contains(NOT_FOUND)) {
            final String msg = MSG_NOT_FOUND + "\n >> " + cmd1 + "\n" + out1;
            throw new IOException(msg);
        }

        //--------------------------------------------------------------------
        // Second, we need to check if we have permissions to run the commands
        final StringBuilder useradd = new StringBuilder();
        useradd.append("sudo ");
        ...
        final String cmd3 = useradd.toString();
        final String out3 = shell.runCommandAndGetOutputMessageAndError(cmd3);

        if (out3.contains(PERMISSION_DENIED)) {
            final String msg = "Permission denied! For command:\n >> " + cmd3 + "\n" + out3;
            throw new IOException(msg);
        }

        // Does the user account have a respective password?
        if (password != null) {
            final String cmd2 = "chpasswd -h";
            final String out2 = shell.runCommandAndGetOutputMessageAndError(cmd2);
            ...
            final StringBuilder chpasswd = new StringBuilder();
            ...
            final String cmd4 = chpasswd.toString();
            final String out4 = shell.runCommandAndGetOutputMessageAndError(cmd4);
     ...
     */
    public static void deleteUser(final String username, final boolean removeHome) throws IOException {
        ...
        final ShellCommander shell = new ShellCommander();
        final String out = shell.runCommandAndGetOutputMessageAndError(cmd.toString());
     ...
     */
    public static void chmod(final boolean sudo, final boolean recursive, final String mode, final String path) throws IOException {
        final StringBuilder cmd = new StringBuilder();
    ...
    public static void chgrp(final boolean recursive, final String newGroup, final String path) throws IOException {
        final StringBuilder cmd = new StringBuilder();
        cmd.append("sudo ");
        cmd.append("chgrp ");
        cmd.append(recursive ? "--recursive " : "");
    ...
    static String findHomeDir(final String username) throws IOException {
        final StringBuilder cmd = new StringBuilder();
