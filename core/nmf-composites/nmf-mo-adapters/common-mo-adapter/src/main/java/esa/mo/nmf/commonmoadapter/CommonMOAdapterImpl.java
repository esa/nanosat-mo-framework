...
public class CommonMOAdapterImpl extends NMFConsumer implements SimpleCommandingInterface
{
  ...
  public void setParameter(final String parameterName, final Serializable content)
  {
    ...
    final IdentifierList parameters = new IdentifierList(1);
    parameters.add(new Identifier(parameterName));

    // If it is java type, then convert it to Attribute
    final Object midValue = HelperAttributes.javaType2Attribute(content);
    final Attribute rawValue;
    ...
    final ParameterStub parameterService = super.getMCServices().getParameterService().getParameterStub();
    ...
    try {
      ...
      final ObjectInstancePair objId = objIds.get(0);
      ...
      if (objId == null) {
        ...
        final ParameterDefinitionDetails parameterDefinition = new ParameterDefinitionDetails();
        ...
        final ParameterCreationRequestList request = new ParameterCreationRequestList(1);
      ...
      // Continues here...
      final ParameterRawValueList raws = new ParameterRawValueList();
      final ParameterRawValue raw = new ParameterRawValue(objIds.get(0).getObjIdentityInstanceId(),
          rawValue);
      raws.add(raw);
          ...
          final Map qosp)
      {
        ...
        if (lParameterValueList.size() == lUpdateHeaderList.size()) {
          for (int i = 0; i < lUpdateHeaderList.size(); i++) {
            final String parameterName = lUpdateHeaderList.get(i).getKey().getFirstSubKey().toString();
            final Attribute parameterValue = lParameterValueList.get(i).getRawValue();
            Serializable object;
            ...
            if (listener instanceof CompleteDataReceivedListener) {
              final ObjectId source = lObjectIdList.get(i);
              final Time timestamp = lUpdateHeaderList.get(i).getTimestamp();

              final ParameterInstance parameterInstance = new ParameterInstance(new Identifier(
                  parameterName),
        ...
        if (lAggregationValueList.size() == lUpdateHeaderList.size()) {
          for (int i = 0; i < lUpdateHeaderList.size(); i++) {
            ...
            if (listener instanceof SimpleAggregationReceivedListener) {
              final List<ParameterInstance> parameterInstances = new LinkedList<>();

              final AggregationValue aggregationValue = lAggregationValueList.get(i);

              for (final AggregationSetValue aggregationSetValue : aggregationValue.getParameterSetValues()) {
                for (final AggregationParameterValue aggregationParamValue : aggregationSetValue.getValues()) {

                  final Long paramDefInstId = aggregationParamValue.getParamDefInstId();
                  final Attribute parameterValue = aggregationParamValue.getValue().getRawValue();
            ...
            if (listener instanceof CompleteAggregationReceivedListener) {
              final ObjectId source = lObjectIdList.get(i);
              final Time timestamp = lUpdateHeaderList.get(i).getTimestamp();
              final String aggregationName = lUpdateHeaderList.get(i).getKey().getFirstSubKey().toString();
              final AggregationValue aggregationValue = lAggregationValueList.get(i);

              final AggregationInstance aggregationInstance = new AggregationInstance(
  ...
  {
    Long actionID = null;
    final IdentifierList actionNames = new IdentifierList(1);
    actionNames.add(new Identifier(actionName));

    final ActionStub actionService = super.getMCServices().getActionService().getActionStub();
    ...
    try {
      ...
      if (objId == null) {
        ...
        final ActionDefinitionDetails actionDefinition = new ActionDefinitionDetails();
        actionDefinition.setDescription(
            "This Definition was automatically generated by: " + CommonMOAdapterImpl.class.getName());
        actionDefinition.setProgressStepCount(new UShort((short) 0));

        final ArgumentDefinitionDetailsList argList = new ArgumentDefinitionDetailsList(objects.length);

        for (final Serializable object : objects) {
          final ArgumentDefinitionDetails argDef = new ArgumentDefinitionDetails();
          // If it is java type, then convert it to Attribute
          final Object midValue = HelperAttributes.javaType2Attribute(object);
          final Attribute rawValue;
        ...
        final ActionCreationRequestList acrl = new ActionCreationRequestList();
      ...
      final AttributeValueList argValues = new AttributeValueList();

      // Fill-in the argument values
      for (final Serializable object : objects) {
        final AttributeValue argValue = new AttributeValue();
        // If it is java type, then convert it to Attribute
        final Object midValue = HelperAttributes.javaType2Attribute(object);
        final Attribute rawValue;
        if (midValue instanceof Attribute) {
    ...
    } catch (final MALInteractionException | NMFException | MALException ex) {
      LOGGER.log(Level.SEVERE, null, ex);
    }
    return actionID;
  }
  ...
  @Override
  public Long launchAction(final Long defInstId, final AttributeValueList argumentValues) throws NMFException {
  ...
  public void closeConnections()
  {
    ...
    if (this.parameterSubscription != null) {
      try {
        ...
        {
          @Override
          public void monitorValueDeregisterAckReceived(final MALMessageHeader msgHeader,
  ...
  @Override
  public Long launchAction(final Long defInstId, final AttributeValueList argumentValues, final ActionAdapter actionAdapter) throws NMFException
  {
    Long instanceObjId = null;

    // Create an ActionInstanceDetails object for the invocation of this action
    final Boolean stageStartedRequired = true;
    final Boolean stageProgressRequired = true;
    final Boolean stageCompletedRequired = true;
    final IdentifierList argumentIds = null;

    try {
      final ActionInstanceDetails instanceDetails =
      ...
      final SingleConnectionDetails actionConnection =
          super.getMCServices().getActionService().getConnectionDetails();

      final ArchiveDetailsList archiveDetailsListActionInstance =
      ...
      final ActionInstanceDetailsList instanceDetailsList = new ActionInstanceDetailsList();
      instanceDetailsList.add(instanceDetails);

      final LongList objIdActionInstances =
      ...
      final MALMessage msg = super.getMCServices().getActionService().getActionStub().asyncSubmitAction(
          instanceObjId,
          instanceDetails,
          actionAdapter
      );
      ...
      final ArchiveDetailsList archiveDetailsListOp = HelperArchive.generateArchiveDetailsList(
          related,
          source,
          actionConnection.getProviderURI());
      ...
      final ObjectId source2 = new ObjectId(ActivityTrackingHelper.OPERATIONACTIVITY_OBJECT_TYPE,
          new ObjectKey(actionConnection.getDomain(), transId));
      final ObjectDetails details = new ObjectDetails(defInstId, source2);
  ...
  /** {@inheritDoc} */
  @Override
  public void toggleParametersGeneration(final List<String> parameterNames, final boolean enableGeneration)
      throws NMFException {
    ...
    try {
      ...
      final ObjectInstancePairList objIds = parameterService.listDefinition(parameters);
      ...
      final InstanceBooleanPairList ibpl = new InstanceBooleanPairList();
      for (final ObjectInstancePair objectInstancePair : objIds) {
