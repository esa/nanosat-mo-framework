...
public class CommonMOAdapterImpl extends NMFConsumer implements SimpleCommandingInterface {
    ...
    public void closeConnections() {
        ...
        if (this.parameterSubscription != null) {
            try {
                ...
                super.getMCServices().getParameterService().getParameterStub().asyncMonitorValueDeregister(idList,
        ...
        if (this.aggregationSubscription != null) {
            try {
                ...
                super.getMCServices().getAggregationService().getAggregationStub().asyncMonitorValueDeregister(idList,
                    new AggregationAdapter() {
                        @Override
                        public void monitorValueDeregisterAckReceived(MALMessageHeader msgHeader, Map qosProperties) {
    ...
    @Override
    public Long launchAction(Long defInstId, AttributeValueList argumentValues, ActionAdapter actionAdapter)
        throws NMFException {
        ...
        try {
            ActionInstanceDetails instanceDetails = new ActionInstanceDetails(defInstId, stageStartedRequired,
                stageProgressRequired, stageCompletedRequired, argumentValues, argumentIds, null);
            ...
            MALMessage msg = super.getMCServices().getActionService().getActionStub().asyncSubmitAction(instanceObjId,
                instanceDetails, actionAdapter);
            ...
            if (objIds.size() < parameterNames.size()) {
                if (objIds.size() <= 0) {
                    ...
                    LOGGER.log(Level.WARNING, String.format(
                        "Couldn't get some parameters instance IDs, for parameters names: %s", paramNames));
                }
            }
