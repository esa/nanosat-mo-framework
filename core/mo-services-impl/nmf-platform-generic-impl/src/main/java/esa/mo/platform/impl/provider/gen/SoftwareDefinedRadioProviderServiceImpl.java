...
public class SoftwareDefinedRadioProviderServiceImpl extends SoftwareDefinedRadioInheritanceSkeleton {
    ...
    public synchronized void init(SoftwareDefinedRadioAdapterInterface adapter) throws MALException {
        ...
        publisher = createStreamRadioPublisher(
    ...
    private void streamRadioUpdate() {
        try {
            ...
            Logger.getLogger(SoftwareDefinedRadioProviderServiceImpl.class.getName()).log(
                    Level.WARNING, "Exception during publishing process on the provider {0}", ex);
        }
    }
    ...
    public synchronized void enableSDR(final Boolean enable,
        ...
        if (!enable) {
            ...
            if (!adapter.setConfiguration(initialConfiguration)) {
                throw new MALInteractionException(new MALStandardError(
                        COMHelper.INVALID_ERROR_NUMBER, null));
            }
            sdrInUse = true;
    ...
    public static final class PublishInteractionListener implements MALPublishInteractionListener {
        ...
        public void publishRegisterAckReceived(final MALMessageHeader header, 
                final Map qosProperties) throws MALException {
            Logger.getLogger(SoftwareDefinedRadioProviderServiceImpl.class.getName()).log(
                    Level.INFO, "Registration Ack: {0}", header.toString());
        }
