...
public class MPServiceOperationHelper {
    ...
    public static MPServiceOperationArguments asArguments(ObjectId identityId, ObjectId definitionId,
                                                          ObjectId instanceId, ObjectId statusId,
                                                          MALInteraction interaction, ObjectId source) {
    ...
    public static List<MPServiceOperationArguments> asArgumentsList(ObjectIdList identityIds,
                                                                    ObjectIdList definitionIds,
                                                                    ObjectIdList instanceIds, ObjectIdList statusIds,
                                                                    MALInteraction interaction, ObjectId source) {
        List<MPServiceOperationArguments> list = new ArrayList<>();
        for (int index = 0; index < identityIds.size(); index++) {
            list.add(new MPServiceOperationArguments(identityIds.get(index), definitionIds != null ?
                definitionIds.get(index) :
                null, instanceIds != null ? instanceIds.get(index) : null, statusIds != null ?
                    statusIds.get(index) :
                    null, interaction, source));
        }
        return list;
    }
