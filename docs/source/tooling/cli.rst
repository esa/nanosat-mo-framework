...
The NMF CLI Tool is a command-line consumer for headless interaction
with the COM Archive, MC services, Platform services, and Software
Management services. It complements the GUI :doc:`ctt` for scripting,
automation, and quick one-off queries.

The CLI is delivered with the SDK under ``sdk/cli-tool`` and is invoked
as:
...
- **apps-launcher** — subscribe to apps' stdout, run, stop, and kill
  apps remotely.
- **heartbeat** — subscribe to a provider's heartbeat.

Each command supports either a **local** mode (``-l <database>``) for
querying a local SQLite COM Archive file, or a **remote** mode
(``-r <providerURI>``) for connecting to a running provider.
...
Each command exposes a ``-h`` / ``--help`` flag that prints its full
option set. The CLI is the right tool for shell scripts, scheduled
jobs, and any operation where opening the CTT is impractical.
